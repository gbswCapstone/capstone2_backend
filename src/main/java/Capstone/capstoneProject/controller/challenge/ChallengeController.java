...
import Capstone.capstoneProject.dto.challenge.ChallengeCreate;
import Capstone.capstoneProject.dto.challenge.ChallengeDetailResponse;
import Capstone.capstoneProject.dto.challenge.ChallengeJoinResponse;
...
import Capstone.capstoneProject.service.challenge.ChallengeService;
...
public class ChallengeController {
    ...
    @Operation(summary = "챌린지방 탈퇴", description = "챌린지방 탈퇴 시 사용하는 API 입니다.")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200", description = "요청 성공"
            ), @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "403", description = "해당 챌린지방 참가자가 아닙니다.",
            content = @Content),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "404", description = "해당 챌린지방을 찾을 수 없습니다.",
                    content = @Content),
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "404", description = "해당 챌린지 채팅방을 찾을 수 없습니다.",
            content = @Content),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "403", description = "로그인이 필요합니다.",
                    content = @Content)
    })
    public ResponseEntity<ApiResponse<Void>> exitChallenge(@PathVariable Long challengeId) {
        challengeService.exitChallenge(challengeId);
