...
import Capstone.capstoneProject.dto.ChallengeLikeResponse;
...
//                    challengeHashtagRepository.findByChallenge(challenges.getId());
//
//            // 기존 해시태그 이름 목록
...
//
//            // 🔹 삭제해야 할 해시태그 관계
//            List<ChallengeHashtag> toDelete = existingRelations.stream()
//                    .filter(ch -> !newNames.contains(ch.getHashtag().getName()))
//                    .toList();
//            challengeHashtagRepository.deleteAll(toDelete);
//
//            // 🔹 추가해야 할 해시태그
//            List<String> toAdd = newNames.stream()
//                    .filter(name -> !existingNames.contains(name))
//                    .toList();
//
//            List<Hashtag> hashtagsToAdd = toAdd.stream()
//                    .map(name -> hashtagRepository.findByName(name)
//                            .orElseGet(() -> hashtagRepository.save(new Hashtag(name))))
//                    .toList();
//
//            List<ChallengeHashtag> newRelations = hashtagsToAdd.stream()
